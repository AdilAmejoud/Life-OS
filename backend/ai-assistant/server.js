...
function ensureFrontendBuilt() {
  const frontendPath = path.join(__dirname, 'frontend', 'pages', 'ai');
...
const server = http.createServer(async (req, res) => {
  ...
  // Serve static files from frontend dist (built Vite app)
  // The frontend is mounted at /app/frontend/pages/ai
  const frontendDistPath = path.join(__dirname, 'frontend', 'pages', 'ai', 'dist');

  const urlPathRaw = (req.url || '/').split('?')[0];
  const urlPath = (() => {
    try {
      return decodeURIComponent(urlPathRaw);
    } catch {
      return urlPathRaw;
    }
  })();

  // Important: `path.join(publicPath, "/assets/x")` would ignore `publicPath`.
  // Also guard against path traversal.
  const resolvedPath = (() => {
    const rel = urlPath === '/' || urlPath === '/index.html'
      ? 'index.html'
      : urlPath.replace(/^\/+/, '');
  ...
  // SPA catch-all (redirect unknown GET requests to index.html if not API)
    ...
    req.on('end', async () => {
      try {
        ...
        if (!convId) {
          ...
          const conv = await db.crud.createConversation(chatTitle, MODEL);
          convId = conv.id;
        } else {
  ...
  // =====================================================
  // Credentials Endpoints (API keys and OAuth tokens)
  // =====================================================
  if (req.method === 'GET' && req.url === '/api/credentials') {
    const service = req.url.split('?').length > 1 ? new URLSearchParams(req.url.split('?')[1]).get('service') : null;
    try {
  ...
  if (req.method === 'GET' && req.url.match(/^\/api\/credentials\/([^/]+)$/)) {
    const service = decodeURIComponent(req.url.split('/')[3]);
    const keyName = decodeURIComponent(req.url.split('/')[4] || '');
    try {
      const credential = keyName
        ? await db.crud.getCredentials(service, keyName)
        : await db.crud.listCredentials(service);
      res.writeHead(200, { 'Content-Type': 'application/json' });
