...
// Migration for schema version 3
function migrateToV3() {
  console.log('Running migration to schema version 3...');
  return new Promise((resolve, reject) => {
    const migrations = [
      // Credentials table - Store API keys and OAuth tokens
      ...
      db.run(query, (err) => {
        ...
        if (completed === migrations.length) {
          if (errors.length > 0) {
            ...
            console.log('Schema version 3 migration completed');
            resolve();
          }
        }
      });
...
module.exports.crud = {
  ...
  // Credentials
  getCredentials: (service, keyName) => {
    return new Promise((resolve, reject) => {
      db.get(
        `SELECT * FROM credentials WHERE service = ? AND key_name = ?`,
        [service, keyName],
        (err, row) => {
          if (err) return reject(err);
          resolve(row);
  ...
  listCredentials: (service) => {
    return new Promise((resolve, reject) => {
      let query = `SELECT id, service, key_name, created_at, updated_at FROM credentials`;
      const params = [];
      if (service) {
        query += ` WHERE service = ?`;
        params.push(service);
  ...
  saveCredential: (service, keyName, value, encrypted) => {
    return new Promise((resolve, reject) => {
      db.run(
        `INSERT OR REPLACE INTO credentials (service, key_name, value, encrypted) VALUES (?, ?, ?, ?)`,
        [service, keyName, value, encrypted !== undefined ? encrypted : 1],
        function (err) {
          if (err) return reject(err);
          resolve({ service, keyName, encrypted });
        }
      );
    });
  },

  deleteCredential: (service, keyName) => {
    return new Promise((resolve, reject) => {
      db.run(
